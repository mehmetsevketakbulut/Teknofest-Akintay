...
void mod2() {
  float pixelX = analogRead(A0) / 4.0 + 1000;
  float pixelY = analogRead(A3) / 4.0 + 1000;

  pixelX = 1500 + (pixelX - 1500) / hizBoleni;
  pixelY = 1500 + (pixelY - 1500) / hizBoleni;

  if (pixelX > maxdeger) pixelX = maxdeger;
  if (pixelY > maxdeger) pixelY = maxdeger;
  if (pixelX < mindeger) pixelX = mindeger;
  if (pixelY < mindeger) pixelY = mindeger;

  Serial.print("pixelX: "); Serial.print(pixelX);
  Serial.print("  pixelY: "); Serial.println(pixelY);

  onsagust_deger = 1500 - (pixelX - 1500) + (pixelY - 1500);
  onsolust_deger = 1500 + (pixelX - 1500) + (pixelY - 1500);
  arsolust_deger = 1500 - (pixelX - 1500) - (pixelY - 1500);
  arsagust_deger = 1500 + (pixelX - 1500) - (pixelY - 1500);
  onsagalt_deger = 1500 - (pixelX - 1500) + (pixelY - 1500);
  onsolalt_deger = 1500 + (pixelX - 1500) + (pixelY - 1500);
  arsolalt_deger = 1500 - (pixelX - 1500) - (pixelY - 1500);
  arsagalt_deger = 1500 + (pixelX - 1500) - (pixelY - 1500);
