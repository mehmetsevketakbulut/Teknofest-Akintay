...
# PCA ile konturun yön açısını (derece cinsinden) hesaplar
...
def line_following():
    ...
    while True:
        ...
        filtered = [cnt for cnt in contours_roi if cv2.contourArea(cnt) > min_area_threshold]
        ...
        if filtered:
            ...
            if M["m00"] != 0:
                ...
                # Kontur yön açısı hesaplanır
                orientation_angle = get_orientation(largest)
                target_angle = 90  # Çizginin hedef açısı diklik
                angle_deviation = abs(abs(orientation_angle) - target_angle)
                angle_align_threshold = 15  # Tolerans
                ...
                # Yön kararı (paralellik mantığına göre)
                if angle_deviation < angle_align_threshold:
            ...
            # ROI'de çizgi yoksa genel görüntüde ara
