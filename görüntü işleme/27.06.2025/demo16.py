...
def scaled_joystick_value(direction, orientation_angle=None, cx_offset=None, area=None):
    ...
    if direction == "SAG DON" and orientation_angle is not None:
        ...
        if area is not None:
            if area < 2000:
                y1 = 1600  # çok uzakta: hızla ilerle
            elif area > 7000:
                y1 = 1450  # çok yakında: yavaşla veya dur
            else:
                y1 = scale(area, 2000, 7000, 1600, 1450)  # orta mesafe: kademeli hız
        else:
            y1 = 1600
