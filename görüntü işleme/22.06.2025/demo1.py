...
def line_following():
    ...
    while True:
        ...
        frame = cv2.flip(frame, 1)  # Ayna düzeltmesi
        ...
        if contours:
            ...
            if M["m00"] != 0:
                cx = int(M["m10"] / M["m00"])
                cy = int(M["m01"] / M["m00"])
                global_cx = cx + roi_left
                global_cy = cy + roi_top

                delta_x = global_cx - roi_center_x
                delta_y = global_cy - roi_center_y

                cv2.putText(frame, f"deltaX: {delta_x}, deltaY: {delta_y}", (50, 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)

                # Çizgi ROI'nin içinde mi?
                if roi_left <= global_cx <= roi_right and roi_top <= global_cy <= roi_bottom:
                    # Normal yön tayini (ekran merkezine göre)
                    frame_center = width // 2
                    if global_cx < frame_center - 50:
                        direction = "SOL"
                    elif global_cx > frame_center + 50:
                        direction = "SAG"
                    else:
                        direction = "DUZ ILERLE"
                else:
                    # Geri getirme yönü: ROI merkezine göre ters yön
                    if global_cx < roi_center_x - 10:
                        direction = "SAG (Geri ROI'ye)"
                    elif global_cx > roi_center_x + 10:
                ...
                cv2.putText(frame, "Cizgi bulundu ama merkez hesaplanamadi", (50, 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        else:
            cv2.putText(frame, "Cizgi bulunamadi", (50, 50),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)

        # ROI dikdörtgenini çiz
