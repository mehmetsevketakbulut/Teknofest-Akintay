...
def line_following():
    ...
    while True:
        ...
        # 2. pencere: şekil tespiti için mask ve kontur çizimleri
        shape_display = frame.copy()
        # ROI mask
        mask_bgr = cv2.cvtColor(mask_roi, cv2.COLOR_GRAY2BGR)
        shape_display[roi_top:roi_bottom, roi_left:roi_right] = mask_bgr

        # Konturları çiz (ROI)
        for cnt in filtered_roi:
            cv2.drawContours(shape_display[roi_top:roi_bottom, roi_left:roi_right], [cnt], -1, (0,255,0), 2)
        
        # Konturları çiz (full frame)
        for cnt in filtered_full:
            cv2.drawContours(shape_display, [cnt], -1, (0,165,255), 2)

        # Pencereleri göster
        cv2.imshow("Underwater Line Following", frame)
        cv2.imshow("Shape Detection View", shape_display)
